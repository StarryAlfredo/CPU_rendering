...
Vec3f PointLight::ColorShader(Vec3f &normal){
	...
	float k = 1.f / (dis * dis);
	float  diffuseFactor = std::max(0.0f, normal * Direction);
	
	Vec3f ambient = ambientK.ColorProduct(LightOfAmbient);
	Vec3f diffuse;
	Vec3f specular;

	Vec3f finalColor = ambient;
	if (diffuseFactor > 0.0f) {
		Vec3f diffuse = diffuseK.ColorProduct(LightOfDiffuse) * diffuseFactor;

		Vec3f h = (Direction + eyePosition);
		h.normalize();
		Vec3f specular = specularK.ColorProduct(LightOfSpecular) * std::pow(std::max(0.f, normal * h), m);
...
Vec3f SpotLight::ColorShader(Vec3f & normal){	
	...
	float k = std::pow(std::max(0.0f, spotDir * Direction),spot);
	float kWithD = k / dis * dis;
	Vec3f diffuse = diffuseK.ColorProduct(LightOfDiffuse) * std::max(0.0f, normal * Direction);
	Vec3f ambient = ambientK.ColorProduct(LightOfAmbient);
	Vec3f h = (Direction + eyePosition);
	h.normalize();
	Vec3f specular = specularK.ColorProduct(LightOfSpecular) * std::pow(std::max(0.f, normal * h), m);
